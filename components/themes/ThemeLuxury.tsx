      ...
      <div className="min-h-screen flex flex-col" dir={lang === 'en' ? 'ltr' : 'rtl'} style={{ backgroundColor: pageTheme.bg, color: '#e8e0d0', fontFamily: headingFont }}>
        ...
        {sections.hero && (
          <section data-section="hero" className={`relative ${isPreview ? 'min-h-[40vh]' : 'min-h-[46vh]'} flex items-center justify-center overflow-hidden`} style={{ order: sectionOrder.hero }}>
            ...
            <div className="absolute inset-0" style={{ background: isPreview ? 'linear-gradient(to bottom, rgba(0,0,0,0.24) 0%, rgba(0,0,0,0.62) 100%)' : 'linear-gradient(to bottom, rgba(0,0,0,0.45) 0%, rgba(0,0,0,0.88) 100%)' }} />
            <div className={`relative z-10 text-center px-6 max-w-5xl mx-auto ${isPreview ? 'pt-10 pb-6' : 'pt-24 pb-16'}`}>
              {profile?.logo_url && (
        ...
        <section data-section="contact" className={`lux-reveal ${isPreview ? 'py-6' : 'py-16'} px-6`} style={{ backgroundColor: '#0d0d0d', order: sectionOrder.contact }}>
          <div className="max-w-xl mx-auto text-center">
            ...
            <div className="space-y-3" dir="ltr">
              {whatsapp && <a href={waLink} target="_blank" rel="noopener noreferrer" dir="ltr" className="flex max-w-full min-w-0 items-center justify-center gap-3 text-sm text-gray-400 hover:text-white transition-colors"><WaIcon className="w-4 h-4 shrink-0" /> <PhoneText value={waDisplay} /></a>}
              {profile?.contact_email && <a href={`mailto:${profile.contact_email}`} className="flex items-center justify-center gap-2 text-sm text-gray-400 hover:text-white transition-colors" dir="ltr"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" className="w-3.5 h-3.5 shrink-0"><path d="M3 4a2 2 0 00-2 2v1.161l8.441 4.221a1.25 1.25 0 001.118 0L19 7.162V6a2 2 0 00-2-2H3z"/><path d="M19 8.839l-7.77 3.885a2.75 2.75 0 01-2.46 0L1 8.839V14a2 2 0 002 2h14a2 2 0 002-2V8.839z"/></svg> {profile.contact_email}</a>}
              {profile?.contact_phone && <a href={`tel:${profile.contact_phone}`} className="flex max-w-full min-w-0 items-center justify-center gap-2 text-sm text-gray-400 hover:text-white transition-colors" dir="ltr"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" className="w-3.5 h-3.5 shrink-0"><path fillRule="evenodd" d="M2 3.5A1.5 1.5 0 013.5 2h1.148a1.5 1.5 0 011.465 1.175l.716 3.223a1.5 1.5 0 01-1.052 1.767l-.933.267c-.41.117-.643.555-.48.95a11.542 11.542 0 006.254 6.254c.395.163.833-.07.95-.48l.267-.933a1.5 1.5 0 011.767-1.052l3.223.716A1.5 1.5 0 0118 15.352V16.5a1.5 1.5 0 01-1.5 1.5A15 15 0 012 3.5z" clipRule="evenodd"/></svg> <PhoneText value={profile.contact_phone} /></a>}
              {(profile?.extra_phones ?? []).filter(Boolean).map((n, i) => (
                <a key={i} href={`tel:${n}`} className="flex max-w-full min-w-0 items-center justify-center gap-2 text-sm text-gray-400 hover:text-white transition-colors" dir="ltr"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" className="w-3.5 h-3.5 shrink-0"><path fillRule="evenodd" d="M2 3.5A1.5 1.5 0 013.5 2h1.148a1.5 1.5 0 011.465 1.175l.716 3.223a1.5 1.5 0 01-1.052 1.767l-.933.267c-.41.117-.643.555-.48.95a11.542 11.542 0 006.254 6.254c.395.163.833-.07.95-.48l.267-.933a1.5 1.5 0 011.767-1.052l3.223.716A1.5 1.5 0 0118 15.352V16.5a1.5 1.5 0 01-1.5 1.5A15 15 0 012 3.5z" clipRule="evenodd"/></svg> <PhoneText value={n} /></a>
              ))}
