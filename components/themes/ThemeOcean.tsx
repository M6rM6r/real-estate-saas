          ...
          <section data-section="contact" className={`ocn-reveal ${isPreview ? 'py-6' : 'py-14'} px-4`} style={{ backgroundColor: pageTheme.sectionAlt, order: sectionOrder.contact }}>
            <div className="max-w-xl mx-auto text-center">
              ...
              <div className="flex flex-col items-center gap-3" dir="ltr">
                {whatsapp && <a href={waLink} target="_blank" rel="noopener noreferrer" className="inline-flex max-w-full flex-wrap items-center gap-2 px-6 py-3 rounded-xl text-white font-semibold text-sm transition-opacity hover:opacity-90" style={{ backgroundColor: '#25D366' }}><WaIcon className="w-5 h-5" /> <span>واتساب:</span> <PhoneText value={waDisplay} /></a>}
                {profile?.contact_phone && <a href={`tel:${profile.contact_phone}`} className="flex max-w-full min-w-0 items-center gap-2 text-sm font-medium text-gray-400 hover:underline" dir="ltr"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" className="w-4 h-4 shrink-0"><path fillRule="evenodd" d="M2 3.5A1.5 1.5 0 013.5 2h1.148a1.5 1.5 0 011.465 1.175l.716 3.223a1.5 1.5 0 01-1.052 1.767l-.933.267c-.41.117-.643.555-.48.95a11.542 11.542 0 006.254 6.254c.395.163.833-.07.95-.48l.267-.933a1.5 1.5 0 011.767-1.052l3.223.716A1.5 1.5 0 0118 15.352V16.5a1.5 1.5 0 01-1.5 1.5A15 15 0 012 3.5z" clipRule="evenodd"/></svg> <PhoneText value={profile.contact_phone} /></a>}
                {(profile?.extra_phones ?? []).filter(Boolean).map((n, i) => (
                  <a key={i} href={`tel:${n}`} className="flex max-w-full min-w-0 items-center gap-2 text-sm font-medium text-gray-400 hover:underline" dir="ltr"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" className="w-4 h-4 shrink-0"><path fillRule="evenodd" d="M2 3.5A1.5 1.5 0 013.5 2h1.148a1.5 1.5 0 011.465 1.175l.716 3.223a1.5 1.5 0 01-1.052 1.767l-.933.267c-.41.117-.643.555-.48.95a11.542 11.542 0 006.254 6.254c.395.163.833-.07.95-.48l.267-.933a1.5 1.5 0 011.767-1.052l3.223.716A1.5 1.5 0 0118 15.352V16.5a1.5 1.5 0 01-1.5 1.5A15 15 0 012 3.5z" clipRule="evenodd"/></svg> <PhoneText value={n} /></a>
                ))}
