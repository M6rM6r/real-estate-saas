...
import {
  ...
  getHeadingFont,
  SocialLinks, WorkingHours, PropertyCard, EmptyCoverPlaceholder, EmptyState, PhoneText, THEME_LABELS,
} from './shared'
          ...
          <section data-section="contact" className={`reveal ${isPreview ? 'py-6 sm:py-7' : 'py-12 sm:py-16'} px-4 md:px-8`} style={{ backgroundColor: pageTheme.sectionAlt, order: sectionOrder.contact }}>
            <div className="max-w-3xl mx-auto rounded-[2rem] border p-6 sm:p-9 text-center" style={{ backgroundColor: pageTheme.cardBg, borderColor: pageTheme.cardBorder, boxShadow: pageTheme.cardShadow }}>
              ...
              <div className="flex flex-col items-center gap-3" dir="ltr">
                {whatsapp && <a href={waLink} target="_blank" rel="noopener noreferrer" className="inline-flex max-w-full flex-wrap items-center justify-center gap-2 px-6 py-3 rounded-xl text-white font-semibold text-sm transition-opacity hover:opacity-90" style={{ backgroundColor: '#25D366' }}><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-5 h-5"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347z"/><path d="M12 0C5.373 0 0 5.373 0 12c0 2.126.555 4.12 1.529 5.856L0 24l6.302-1.508A11.947 11.947 0 0012 24c6.627 0 12-5.373 12-12S18.627 0 12 0zm0 21.818a9.798 9.798 0 01-5.021-1.378l-.36-.213-3.741.895.929-3.631-.234-.375A9.788 9.788 0 012.182 12C2.182 6.565 6.565 2.182 12 2.182S21.818 6.565 21.818 12 17.435 21.818 12 21.818z"/></svg> <span>واتساب:</span> <PhoneText value={waDisplay} /></a>}
                {profile?.contact_phone && <a href={`tel:${profile.contact_phone}`} className={`flex max-w-full min-w-0 items-center gap-2 text-sm font-medium ${mutedClass} hover:underline`} dir="ltr"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" className="w-4 h-4 shrink-0"><path fillRule="evenodd" d="M2 3.5A1.5 1.5 0 013.5 2h1.148a1.5 1.5 0 011.465 1.175l.716 3.223a1.5 1.5 0 01-1.052 1.767l-.933.267c-.41.117-.643.555-.48.95a11.542 11.542 0 006.254 6.254c.395.163.833-.07.95-.48l.267-.933a1.5 1.5 0 011.767-1.052l3.223.716A1.5 1.5 0 0118 15.352V16.5a1.5 1.5 0 01-1.5 1.5A15 15 0 012 3.5z" clipRule="evenodd"/></svg> <PhoneText value={profile.contact_phone} /></a>}
          ...
          <footer className={`text-white ${isPreview ? 'pb-4 sm:pb-0' : 'pb-28 sm:pb-0'} safe-pb`} style={{ backgroundColor: pageTheme.bg, order: sectionOrder.footer }}>
            ...
            <div className={`max-w-7xl mx-auto px-4 md:px-8 grid grid-cols-1 md:grid-cols-3 ${isPreview ? 'pt-5 pb-4 gap-4' : 'pt-12 pb-10 gap-10'}`}>
              ...
              <div>
                ...
                {whatsapp && (
                  <a href={waLink} target="_blank" rel="noopener noreferrer" className="flex max-w-full min-w-0 items-center gap-2 text-sm text-slate-400 hover:text-white transition-colors mb-2.5" dir="ltr">
                    ...
                    <PhoneText value={waDisplay} />
                  </a>
                )}
                {profile?.contact_phone && (
                  <a href={`tel:${profile.contact_phone}`} className="flex max-w-full min-w-0 items-center gap-2 text-sm text-slate-400 hover:text-white transition-colors mb-2.5" dir="ltr">
                    ...
                    <PhoneText value={profile.contact_phone} />
                  </a>
                )}
              ...
              {/* Social + Hours */}
              <div>
                <h4 className="text-xs font-bold uppercase tracking-[0.2em] mb-4" style={{ color: primary }}>{L.footerSocial}</h4>
