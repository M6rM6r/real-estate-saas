...
export function getPageSections(profile: Profile) {
  return {
    ...
    contact: true,
    working_hours: true,
    order: ['hero', 'listings', 'about', 'contact', 'working_hours', 'footer'] as const,
    ...(profile?.page_sections ?? {}),
    // Global decision: hide news section across public pages.
    news: false,
  }
}
...
export function PhoneText({ value, className = '' }: { value?: string | null; className?: string }) {
  ...
  return (
    <span
      dir="ltr"
      className={`inline-block max-w-full min-w-0 break-all text-left [unicode-bidi:plaintext] ${className}`.trim()}
