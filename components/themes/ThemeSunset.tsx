...
export default function ThemeSunset({ tenant, profile, listings, news, gallery: _gallery, team: _team }: ThemePageProps) {
  const primary = tenant.primary_color ?? '#f97316'
  const pageTheme = PAGE_THEMES['sunset'] ?? PAGE_THEMES.modern
  const isDark = false
  ...
  useEffect(() => {
    const observer = new IntersectionObserver(
      (entries) => entries.forEach((e) => { if (e.isIntersecting) e.target.classList.add('sun-revealed') }),
      { threshold: 0.08 }
    )
    document.querySelectorAll('.sun-reveal').forEach((el) => observer.observe(el))
  ...
  return (
    <>
      <style>{`
        :root { --sun-primary: ${primary}; }
        .sun-reveal { opacity: 0; transform: translateY(24px); transition: opacity 0.6s ease, transform 0.6s ease; }
        .sun-revealed { opacity: 1; transform: translateY(0); }
      ...
      <div className="min-h-screen" dir="rtl" style={{ backgroundColor: pageTheme.bg, color: '#1c1917' }}>
          ...
          <nav className={`transition-all duration-300 ${scrolled ? 'border-b shadow-sm' : 'bg-transparent'}`}
            ...
            <div className="max-w-7xl mx-auto px-4 h-14 sm:h-16 flex items-center justify-between gap-3">
              <div className="flex items-center gap-2.5">
                {profile?.logo_url ? (
                  <Image src={profile.logo_url} alt={tenant.name} width={40} height={40} className="w-9 h-9 rounded-full object-cover ring-2" style={{ ringColor: primary } as React.CSSProperties} />
                ) : (
                  <div className="w-9 h-9 rounded-full flex items-center justify-center font-bold shrink-0 text-white" style={{ backgroundColor: primary }}>
                    {tenant.name.charAt(0)}
                  </div>
                )}
                <span className={`font-bold text-sm sm:text-base drop-shadow-sm ${scrolled ? 'text-gray-900' : 'text-white'}`}>{tenant.name}</span>
              </div>
            </div>
          </nav>
        </header>
          ...
          <section className={`min-h-screen flex flex-col lg:flex-row items-stretch ${bannerPt}`}>
            <div className="relative flex-1 min-h-[40vh] lg:min-h-screen">
              {profile?.cover_url
                ...
                : <div className="w-full h-full min-h-[40vh]" style={{ background: `linear-gradient(135deg, ${primary}cc 0%, #fff5ee 100%)` }} />
              }
            </div>
            <div className="flex-1 flex flex-col justify-center px-8 sm:px-12 py-16" style={{ backgroundColor: pageTheme.bg }}>
              {profile?.logo_url && <Image src={profile.logo_url} alt={tenant.name} width={80} height={80} className="w-16 h-16 object-contain rounded-full mb-6 shadow" />}
              <div className="w-10 h-1 rounded-full mb-5" style={{ backgroundColor: primary }} />
              <h1 className="text-3xl sm:text-5xl font-bold mb-4 leading-tight text-gray-900">{tenant.name}</h1>
              {profile?.tagline && <p className="text-lg font-medium mb-3" style={{ color: primary }}>{profile.tagline}</p>}
              <p className="text-base mb-8 text-gray-500">{pageConfig.hero_headline}</p>
            </div>
          </section>
        )}
        ...
        {sections.hero && pageConfig.hero_style === 'minimal' && (
          <section className={`pb-16 px-4 text-center ${hasBanner ? 'pt-[128px] sm:pt-[140px]' : 'pt-28 sm:pt-32'}`} style={{ backgroundColor: pageTheme.sectionAlt }}>
            {profile?.logo_url && <Image src={profile.logo_url} alt={tenant.name} width={96} height={96} className="w-20 h-20 mx-auto rounded-full object-contain mb-6 shadow-md" />}
            <h1 className="text-4xl sm:text-6xl font-bold mb-4 leading-tight text-gray-900">{tenant.name}</h1>
            ...
            <p className="text-base sm:text-lg mb-8 max-w-xl mx-auto text-gray-500">{pageConfig.hero_headline}</p>
          </section>
        )}
        ...
        {sections.hero && (!pageConfig.hero_style || pageConfig.hero_style === 'centered') && (
          <section className={`relative min-h-[100dvh] flex flex-col items-center justify-end pb-10 sm:pb-20 bg-cover bg-center ${hasBanner ? 'pt-[124px] sm:pt-[136px]' : 'pt-24 sm:pt-28'}`}
            style={{ background: profile?.cover_url ? `url(${profile.cover_url}) center/cover no-repeat, linear-gradient(135deg, ${primary}44 0%, #fff5ee 100%)` : `linear-gradient(135deg, ${primary}55 0%, #fff5ee 55%, ${primary}22 100%)` }}>
            ...
            <div className="relative z-10 text-center px-4 max-w-2xl mx-auto w-full">
              <div className="border rounded-2xl px-5 sm:px-8 py-7 sm:py-10 shadow-xl mx-auto backdrop-blur-sm"
                style={{ backgroundColor: pageTheme.heroCardBg, borderColor: pageTheme.heroCardBorder }}>
                {profile?.logo_url && <Image src={profile.logo_url} alt={tenant.name} width={96} height={96} className="w-20 h-20 object-contain mx-auto mb-4 rounded-full bg-white p-1 shadow-md" priority />}
                <h1 className="text-3xl sm:text-5xl md:text-6xl font-bold mb-3 leading-tight tracking-tight text-gray-900">{tenant.name}</h1>
                {profile?.tagline && <p className="text-base sm:text-xl font-medium mb-3" style={{ color: primary }}>{profile.tagline}</p>}
                <p className="text-sm sm:text-lg text-gray-600 mb-6">{pageConfig.hero_headline}</p>
              </div>
            </div>
        ...
        {sections.listings && published.length > 0 && (
          <section className="sun-reveal py-12 sm:py-16 px-4 md:px-8 max-w-7xl mx-auto">
            <h2 className="text-2xl sm:text-3xl font-bold mb-2 text-gray-900">قائمة العقارات</h2>
            <div className="w-10 h-1 rounded-full mb-6" style={{ backgroundColor: primary }} />
            {pageConfig.show_listing_search && (
              <div className="mb-4">
                <input value={listingSearch} onChange={e => setListingSearch(e.target.value)}
                  ...
                  className="w-full border px-4 py-2.5 text-sm text-gray-900 placeholder-gray-400 bg-white focus:outline-none focus:ring-2"
                ...
                <div className="flex gap-2 overflow-x-auto scrollbar-hide pb-1 -mx-4 px-4 sm:mx-0 sm:px-0">
                  {(['all', 'sale', 'rent'] as const).map(f => (
                    <button key={f} type="button" onClick={() => setOfferFilter(f)}
                      ...
                      style={offerFilter === f ? { backgroundColor: primary, borderColor: primary, color: '#fff', boxShadow: `0 4px 12px ${primary}44` } : { backgroundColor: '#fff', borderColor: pageTheme.cardBorder, color: '#374151' }}>
                  ...
                  <div className="flex gap-2 overflow-x-auto scrollbar-hide pb-1 -mx-4 px-4 sm:mx-0 sm:px-0">
                    {(['all', ...propertyTypes]).map(f => (
                      <button key={f} type="button" onClick={() => setTypeFilter(f)}
                        ...
                        style={typeFilter === f ? { backgroundColor: primary, borderColor: primary, color: '#fff' } : { backgroundColor: '#fff', borderColor: pageTheme.cardBorder, color: '#374151' }}>
                        {f === 'all' ? 'كل الأنواع' : f}
                      </button>
                    ))}
                  </div>
                ...
                <div className="flex items-center gap-2 overflow-x-auto scrollbar-hide pb-1 -mx-4 px-4 sm:mx-0 sm:px-0">
                  {(['all', 'available', 'sold', 'rented'] as const).map(f => (
                    <button key={f} type="button" onClick={() => setStatusFilter(f)}
                      className="shrink-0 px-3 py-1.5 min-h-[36px] rounded-full text-xs font-medium transition-all border active:scale-95"
                      style={statusFilter === f ? { backgroundColor: primary, borderColor: primary, color: '#fff' } : { backgroundColor: '#fff', borderColor: pageTheme.cardBorder, color: '#374151' }}>
                      {f === 'all' ? 'كل الحالات' : STATUS_LABELS[f]}
                    </button>
                  ))}
                  <select value={sortPrice} onChange={e => setSortPrice(e.target.value as 'none' | 'asc' | 'desc')}
                    className="shrink-0 mr-auto text-xs px-3 py-2 border rounded-full focus:outline-none cursor-pointer bg-white text-gray-700"
                    style={{ borderColor: pageTheme.cardBorder }}>
        ...
        {sections.news && news.length > 0 && (
          <section className="sun-reveal py-12 sm:py-16" style={{ backgroundColor: pageTheme.sectionAlt }}>
            <div className="px-4 md:px-8 max-w-7xl mx-auto">
              <h2 className="text-2xl sm:text-3xl font-bold mb-2 text-gray-900">آخر الأخبار</h2>
              <div className="w-10 h-1 rounded-full mb-8" style={{ backgroundColor: primary }} />
              <div className="flex gap-4 overflow-x-auto pb-4 snap-x -mx-4 px-4 sm:mx-0 sm:px-0">
                {news.map(item => (
                  <div key={item.id} className="overflow-hidden shrink-0 snap-start w-[80vw] sm:w-80 border text-gray-900" style={cardStyle}>
                    ...
                    <div className="p-5">
                      <p className="text-xs mb-1 text-gray-400">{new Date(item.created_at).toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })}</p>
                      <h3 className="font-semibold mb-2 text-gray-900">{item.title}</h3>
                      {item.body && <p className="text-sm line-clamp-3 text-gray-500">{item.body}</p>}
                    </div>
                  </div>
                ))}
              </div>
        ...
        {/* About */}
        {sections.about && (
          <section className="sun-reveal py-12 sm:py-16 px-4 md:px-8 max-w-3xl mx-auto text-center">
            <h2 className="text-2xl sm:text-3xl font-bold mb-2 text-gray-900">من نحن</h2>
            <div className="w-10 h-1 rounded-full mx-auto mb-6" style={{ backgroundColor: primary }} />
            {profile?.bio && <p className="leading-relaxed text-base sm:text-lg text-gray-600">{profile.bio}</p>}
            {(profile?.licence_numbers && profile.licence_numbers.length > 0)
              ? profile.licence_numbers.map((l, i) => <p key={i} className="mt-2 text-xs font-mono text-gray-400">{l.label ? `${l.label}: ` : 'رقم الترخيص: '}{l.number}</p>)
        ...
        {(whatsapp || profile?.contact_phone || profile?.contact_email) && (
          <section className="sun-reveal py-12 sm:py-16 px-4 md:px-8" style={{ backgroundColor: pageTheme.sectionAlt }}>
            <div className="max-w-xl mx-auto text-center">
              <h2 className="text-2xl sm:text-3xl font-bold mb-2 text-gray-900">تواصل معنا</h2>
              <div className="w-10 h-1 rounded-full mx-auto mb-6" style={{ backgroundColor: primary }} />
              <p className="text-sm mb-8 text-gray-500">نسعد بخدمتك — تواصل معنا عبر:</p>
              <div className="flex flex-col items-center gap-3" dir="ltr">
                ...
                {profile?.contact_phone && <a href={`tel:${profile.contact_phone}`} className="flex items-center gap-2 text-sm font-medium text-gray-500 hover:underline">{profile.contact_phone}</a>}
                {profile?.contact_email && <a href={`mailto:${profile.contact_email}`} className="flex items-center gap-2 text-sm font-medium text-gray-500 hover:underline">{profile.contact_email}</a>}
              </div>
            </div>
          </section>
        )}
        ...
        {profile?.working_hours && Object.values(profile.working_hours).some(h => h?.enabled) && (
          <section className="sun-reveal py-10 px-4 md:px-8">
        ...
        {/* Footer */}
        {sections.footer && (
          <footer className="py-10 sm:py-12 px-4 md:px-8 pb-28 sm:pb-12 safe-pb text-gray-700" style={{ backgroundColor: '#1c1412', borderTop: `3px solid ${primary}` }}>
            <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-8">
              <div>
                ...
                {profile?.contact_address && <p className="text-gray-400 text-sm mt-3">{profile.contact_address}</p>}
              </div>
              <div>
                <h4 className="font-semibold mb-3 text-xs uppercase tracking-wider text-gray-500">التواصل</h4>
                {whatsapp && <a href={waLink} target="_blank" rel="noopener noreferrer" className="text-sm text-gray-400 hover:text-white block mb-2">{waDisplay}</a>}
                {profile?.contact_email && <a href={`mailto:${profile.contact_email}`} className="text-sm text-gray-400 hover:text-white block mb-2">{profile.contact_email}</a>}
                {profile?.contact_phone && <a href={`tel:${profile.contact_phone}`} className="text-sm text-gray-400 hover:text-white block mb-2">{profile.contact_phone}</a>}
                {(profile?.extra_phones ?? []).filter(Boolean).map((n, i) => (
                  <a key={i} href={`tel:${n}`} className="text-sm text-gray-400 hover:text-white block mb-2">{n}</a>
                ))}
                <div className="mt-4"><SocialLinks profile={profile} waLink={waLink} /></div>
              </div>
              {profile?.working_hours && (
                <div>
                  <h4 className="font-semibold mb-3 text-xs uppercase tracking-wider text-gray-500">ساعات العمل</h4>
                  <WorkingHours hours={profile.working_hours} />
                </div>
              )}
            </div>
