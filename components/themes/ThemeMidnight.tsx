...
import {
  ...
  getHeadingFont,
  SocialLinks, WorkingHours, PropertyCard, EmptyCoverPlaceholder, EmptyState, WaIcon, ListingBadges, PhoneText, THEME_LABELS,
} from './shared'
              ...
              {sections.contact && hasContactInfo && (
                <div data-section="contact" className="border rounded-2xl p-6 sm:p-8 text-center" style={cardStyle}>
                  ...
                  <div className="flex flex-col items-center gap-3" dir="ltr">
                    {whatsapp && <a href={waLink} target="_blank" rel="noopener noreferrer" className="inline-flex max-w-full flex-wrap items-center justify-center gap-2 px-6 py-3 rounded-xl text-white font-semibold text-sm hover:opacity-90 transition-opacity" style={{ backgroundColor: '#25D366' }}>
                      ...
                      <span>واتساب:</span> <PhoneText value={waDisplay} />
                    </a>}
                    {profile?.contact_phone && <a href={`tel:${profile.contact_phone}`} className="flex max-w-full min-w-0 items-center gap-2 text-sm font-medium text-slate-400 hover:underline" dir="ltr"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" className="w-4 h-4 shrink-0"><path fillRule="evenodd" d="M2 3.5A1.5 1.5 0 013.5 2h1.148a1.5 1.5 0 011.465 1.175l.716 3.223a1.5 1.5 0 01-1.052 1.767l-.933.267c-.41.117-.643.555-.48.95a11.542 11.542 0 006.254 6.254c.395.163.833-.07.95-.48l.267-.933a1.5 1.5 0 011.767-1.052l3.223.716A1.5 1.5 0 0118 15.352V16.5a1.5 1.5 0 01-1.5 1.5A15 15 0 012 3.5z" clipRule="evenodd"/></svg> <PhoneText value={profile.contact_phone} /></a>}
                    {(profile?.extra_phones ?? []).filter(Boolean).map((n, i) => (
                      <a key={i} href={`tel:${n}`} className="flex max-w-full min-w-0 items-center gap-2 text-sm font-medium text-slate-400 hover:underline" dir="ltr"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" className="w-4 h-4 shrink-0"><path fillRule="evenodd" d="M2 3.5A1.5 1.5 0 013.5 2h1.148a1.5 1.5 0 011.465 1.175l.716 3.223a1.5 1.5 0 01-1.052 1.767l-.933.267c-.41.117-.643.555-.48.95a11.542 11.542 0 006.254 6.254c.395.163.833-.07.95-.48l.267-.933a1.5 1.5 0 011.767-1.052l3.223.716A1.5 1.5 0 0118 15.352V16.5a1.5 1.5 0 01-1.5 1.5A15 15 0 012 3.5z" clipRule="evenodd"/></svg> <PhoneText value={n} /></a>
                    ))}
          ...
          <footer className={`${isPreview ? 'py-5 sm:py-6 pb-6 sm:pb-6' : 'py-10 sm:py-12 pb-28 sm:pb-12'} px-4 md:px-8 safe-pb`} style={{ backgroundColor: '#080612', borderTop: `1px solid ${pageTheme.navBorder}`, order: sectionOrder.footer }}>
            <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-8">
              ...
              <div>
                ...
                {whatsapp && <a href={waLink} target="_blank" rel="noopener noreferrer" className="block text-sm text-slate-400 hover:text-white mb-2"><PhoneText value={waDisplay} /></a>}
