...
export async function POST(request: NextRequest) {
  ...
  const id = uuidv4()
  const doc = { tenantId: session.tenantId, type: 'news', title, body: description, images: images ?? [], published: !!published, publishedAt: published ? new Date() : null, createdAt: new Date(), ...rest }
