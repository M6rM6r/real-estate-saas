...
export default function AnnouncementsPage() {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    if (modal === 'create') {
      await fetch('/api/dashboard/announcements', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${session.access_token}` },
        body: JSON.stringify(payload),
      })
    } else if (editing) {
      await fetch(`/api/dashboard/announcements/${editing.id}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${session.access_token}` },
        body: JSON.stringify(payload),
      })
  ...
  return (
    <div className="p-6">
      ...
      <div className="space-y-4">
        ...
        {!announcements.length && <p className="text-center text-gray-400 py-12">No announcements yet.</p>}
      </div>
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
            <div className="p-6 border-b flex items-center justify-between">
              <h2 className="text-lg font-bold">{modal === 'create' ? 'New Announcement' : 'Edit Announcement'}</h2>
            ...
            <form onSubmit={handleSave} className="p-6 space-y-4">
              ...
              <div className="flex items-center gap-2">
                <input type="checkbox" id="published_announcement" checked={form.published} onChange={e => setForm(p => ({ ...p, published: e.target.checked }))} className="rounded" />
                <label htmlFor="published_announcement" className="text-sm text-gray-700">Publish announcement</label>
              </div>
